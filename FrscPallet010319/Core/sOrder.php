...
//        $user_group = $user['additional']['user']['customergroup'];
        ...
        if ($user_group === 'H') {
            ...
            $last_inserted = $this->db->fetchAll($sql);
            ...
            if($last_inserted[0]["comment"] === 'pallet'){
                ...
                $shipping_price = $this->getPrice($zip, $pallets);
                ...
                if ($box['boxes'] >= 1) {
                    ...
                    if ($box['remTotal'] >= 900 && $box['remTotal'] <= 999) {
                        $box['boxes'] += 1;
                        $shipping_price = $config['b2bUserShippingCost'] * $box['boxes'];
                        $shipping_price_net = $config['b2bUserShippingCost'] * $box['boxes'];
                    } elseif ($box['remTotal'] == 0) {
                        $shipping_price = $config['b2bUserShippingCost'] * $box['boxes'];
                        $shipping_price_net = $config['b2bUserShippingCost'] * $box['boxes'];
                    }
                }else{
                    $shipping_price = $config['b2bUserShippingCost'];
                    $shipping_price_net = $config['b2bUserShippingCost'];
                }
            }
        }else{
            ...
            if ($box['boxes'] >= 1){
                if ($box['remTotal'] >= 500 && $box['remTotal'] < 790) {
                    $frozen = true;
                    $shipping_price = $config['boxShippingCost'];
                    $shipping_price_net = $config['boxShippingCost'];
                }
            }
        }

        $ammount = str_replace(',','.',$this->sBasketData["AmountWithTaxNumeric"]);
        $total_amount = $shipping_price+$ammount;
        ...
        if ($frozen){
            $orderParams = array(
                ...
                'invoice_amount_net'   => $this->sBasketData["AmountNetNumeric"],
                'invoice_shipping'     => $shipping_price,
                'invoice_shipping_net' => $shipping_price_net,
                'ordertime'            => new Zend_Db_Expr('NOW()'),
                'status'               => 0,
                'cleared'              => 17,
                'paymentID'            => $this->getPaymentId(),
        ...
        if ($frozen){
            $shipping_price_formated = $shipping_price . ' EUR';
            $total_amount_formated = $total_amount . ' EUR';
            $variables = array(
                ...
                "sShippingCosts"=>$shipping_price_formated,
                "sAmount"=>$total_amount_formated,
