    ...
    <div class="ajax--cart">
        ...
        {block name='frontend_checkout_ajax_cart_alert_box'}
            {if $theme.offcanvasCart}
                ...
                {/if}
                {if $boxcount['remTotal'] < 699 && $boxcount['remTotal'] > 0}
                    <div class="alert is--info is--rounded">
                        <div class="alert--icon">
                        <div class="icon--element icon--warning"></div>
                        </div>
                        {if $b2bUser}
                            <div class="alert--content">Wir versenden ausschließlich zu 100% gefüllte 25kg Kartons.</div>
                        {else}
                            <div class="alert--content">Die Mindestbestellmenge beträgt 10kg.</div>
                        {/if}
                    </div>
                {elseif $boxcount['remTotal'] < 800 && $boxcount['remTotal'] > 699}
                    <div class="alert is--info is--rounded">
                        <div class="alert--icon">
                            <div class="icon--element icon--warning"></div>
                        </div>
                        {if $b2bUser}
                            <div class="alert--content">Wir versenden ausschließlich zu 100% gefüllte 25kg Kartons.</div>
                        {else}
                            <div class="alert--content">Bitte fülle die weitere Box mindestens bis 80%</div>
                        {/if}
                    </div>
                {/if}
                {if $sumBoxes}
                ...
                <div class="prices--container">
                    {block name='frontend_checkout_ajax_cart_prices_container_inner'}
                        ...
                        <div class="prices--articles">
                            ...
                            <span class="prices--articles-amount">{$dostava|currency}</span>
                        </div>
                        {if $b2bUser == false}
                            <div class="not-full-box" style="width: 109%;margin-left: -10px">
                                <div class="alert is--info is--rounded">
                                    ...
                                    <div class="alert--content">Für eine nicht zu 100% befüllte Box berechnen wir {$dostava|currency} Versandkosten</div>
                                </div>
                            </div>
                        {/if}
                        <div class="prices--articles">
